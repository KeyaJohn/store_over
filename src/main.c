...
static int get_path()
{
    ...
    snprintf(conf_path, MAX_PATH, "%s/%s", buff, CONF_PATH);
    //printf("conf_path:[%s]\n", conf_path);

    return 0;
}

static int init_wthr()
{
    int i = 0, j = 0;
    int disk_id = 0;
...
static int init_data(char *conf)
{
    if (open_conf(conf) < 0) {
        return -1; 
    }
    ...
    if (get_val_arry("base.rcpu_id", (void **)&def_info->rcpu_id, def_info->rthr_num, TYPE_INT) < 0) {
        return -1;
    }
    def_info->wcpu_id = (int *)malloc(sizeof(int) * def_info->wthr_num);
    if (get_val_arry("base.wcpu_id", (void**)&def_info->wcpu_id, def_info->wthr_num, TYPE_INT) < 0) {
        return -1;
    }
    def_info->scpu_id = (int *)malloc(sizeof(int) * def_info->sthr_num);
    if (get_val_arry("base.scpu_id", (void**)&def_info->scpu_id, def_info->sthr_num, TYPE_INT) < 0) {
        return -1;
    }
...
int start_ctrl()
{
    if (ctrl_init(def_info->ctrl_file) < 0) {
        return -1;
    }
